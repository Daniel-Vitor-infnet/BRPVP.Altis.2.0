...
BRPVP_carrierMissMainCode = {
	if !(BRPVP_carrierMissMainCodeRunning) then {
		...
		while {_found isEqualTo [] && _count <= 200} do {
			...
			private _objs = nearestObjects [_pos,["FlagCarrier","Land_Carrier_01_base_F","Land_Destroyer_01_base_F"],1250,true];
			if (_objs isEqualTo []) then {_found = _pos;};
			_count = _count+1;
		};
		if (_found isEqualTo []) then {_found = selectRandom BRPVP_carrierMissPositions;};
		BRPVP_carrierMissPositions deleteAt (BRPVP_carrierMissPositions find _found);
		
		//CREATE CARRIER
		BRPVP_carrierMissCreateObj = nil;
		[_found,2] remoteExecCall ["BRPVP_carrierMissCreate",2];
		waitUntil {!isNil "BRPVP_carrierMissCreateObj"};
		_carrier = BRPVP_carrierMissCreateObj;
		...
		_cPositions = _cPositions apply {_carrier modelToWorld _x};
		_classes = [];
		_q1 = 5;
		_q2 = 3;
		...
		_start2 = [-(_start1 select 0),-(_start1 select 1),_start1 select 2];
		{
			...
			_return = [_found vectorAdd _placeVec,0,_class,_grp] call BIS_fnc_spawnVehicle;
			...
			_wpPos = _found vectorAdd _extraRad;
			_wpPos set [2,_h];
		...
		//SPAWN BOATS
		{
			...
			_aRnd = random 360;
			_grp = createGroup BLUFOR;
			_return = [[0,0,0],0,_class,_grp] call BIS_fnc_spawnVehicle;
			_veh = _return select 0;
			_veh setVariable ["brpvp_can_disable",true,2];
			_posI = _found getPos [_rad,300+_aRnd];
			_posI set [2,0];
			...
			for "_a" from 0 to 300 step 60 do {
				_wpPos = _found getPos [_rad,_a+_aRnd];
				_wpPos set [2,0];
				_wp = _grp addWayPoint [_wpPos,0];
			...
			[325,"B_T_Boat_Armed_01_minigun_F"]
		];
		
		//FURNITURE
		_dirC = getDir _carrier;
		_furniture = [
			["B_Plane_Fighter_01_F",[38.1172,74.3496,23.5451],_dirC+175],
			["O_Plane_CAS_02_dynamicLoadout_F",[-22.4434,-50.8281,23.5305],_dirC+175],
			["B_Heli_Attack_01_dynamicLoadout_F",[-24.2217,38.2773,23.5299],_dirC+85]
		];
		{
			_x params ["_class","_posRel","_dir"];
			private _fur = createVehicle [_class,_carrier modelToWorld _posRel,[],0,"CAN_COLLIDE"];
			_fur setDir _dir;
			uiSleep BRPVP_simpleMissSpawnWait;
		} forEach _furniture;		
