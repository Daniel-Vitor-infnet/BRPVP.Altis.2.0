...
BRPVP_eventFortOfLightOn = {
	...
	if (_state isEqualTo 0) then {
		...
		BRPVP_eventFortOfLightObjects = _svcObjs;
		BRPVP_eventFortOfLightObjectsHide = [];
		BRPVP_eventFortOfLightGroups = [];
		BRPVP_eventFortOfLightDead = [];
		BRPVP_eventFortOfLightWayPoints = [];
		{
			_pos = ASLToAGL getPosASL _x;
			_pos set [2,0];
			_obj = createVehicle ["Land_runway_edgelight_blue_F",_pos,[],0,"CAN_COLLIDE"];
			BRPVP_eventFortOfLightObjects pushBack _obj;
			BRPVP_eventFortOfLightObjectsHide pushBack _x;
			[_x,true] remoteExecCall ["hideObjectGlobal",2];
		} forEach ([23148,18656,0] nearObjects ["Land_LandMark_F",300]);
		...
		_vehicles = [
			["B_LSV_01_armed_F",100000,["B_soldier_AT_F"]],
			["B_LSV_01_armed_F",100000,["B_soldier_AA_F"]]
		];
		_HMGS = [
			[[23209,18485,17.8909],180],
			[[23502.1,18481.4,17.8909],90],
			[[23510.4,18865,17.8909],90],
			[[23220.1,18861,17.8909],0]
		];
		...
		_addToarray = [];
		{
			...
			BRPVP_eventFortOfLightGroups pushBack _grp;
			BRPVP_eventFortOfLightGroups pushBack _grpAI;
			...
			BRPVP_eventFortOfLightObjects pushBack _veh;
			_veh setVariable ["mmny",_money,false];
			...
			{
				_x addEventHandler ["Killed",{BRPVP_eventFortOfLightDead pushBack (_this select 0);_this call BRPVP_botDaExp;_this call BRPVP_rolaMotorista;}];
			...
			_addToarray append _units;
			{
				...
				BRPVP_eventFortOfLightWayPoints pushBack _wp;
		...
		for "_i" from 1 to 2 do {
			...
			_spawn = _AICheckPoints select 0;
			{
				...
				_unit addEventHandler ["Killed",{BRPVP_eventFortOfLightDead pushBack (_this select 0);_this call BRPVP_botDaExp;}];
		...
		_houses = [];
		_housesUsed = [];
		{if (count (_x buildingPos -1) > 0 && typeOf _x isNotEqualTo "Land_Cargo_House_V3_F") then {_houses pushBack _x;};} forEach (_center nearObjects ["House",_radius]);
		for "_i" from 1 to selectRandom [1] do {
			...
			BRPVP_eventFortOfLightObjects pushBack _box;
		...
		{BRPVP_eventFortOfLightObjects pushBack _x;} forEach (_args call BRPVP_spawnSpecialBox);
		...
		_allSuitCase = [];
		{
			...
			BRPVP_eventFortOfLightObjects pushBack _case;
		...
		} forEach [3000000,3000000];
		{
			...
			_grp = createGroup [BLUFOR,true];
			BRPVP_eventFortOfLightGroups pushBack _grp;
			_return = [[random 100,0,0],0,"B_HMG_01_high_F",_grp] call BIS_fnc_spawnVehicle;
			_hmg = _return select 0;
			_hmg setVariable ["brpvp_can_disable",true,2];
			BRPVP_eventFortOfLightObjects pushBack _hmg;
		...
		[_idx,_allSuitCase] spawn {
			scriptName "EV3 MISSION WAITUNTIL";
			...
			_start2 = [-(_start1 select 0),-(_start1 select 1),_start1 select 2];
			{
				...
				waitUntil {
					...
					count BRPVP_eventFortOfLightDead >= _toKill || {isNull _x} count _allSuitCase > 0 || !_running
				};
				...
				BRPVP_eventFortOfLightObjects pushBack _veh;
				{
					_x addEventHandler ["Killed",{BRPVP_eventFortOfLightDead pushBack (_this select 0);_this call BRPVP_botDaExp;}];
					_x addEventHandler ["HandleDamage",{_this call BRPVP_hdeh}];
				...
				BRPVP_eventFortOfLightWayPoints pushBack _wp;
				_wp setWayPointType "CYCLE";
			} forEach [[5,[200,0,0],_start1,selectRandom ["O_Heli_Attack_02_F","O_Heli_Attack_02_black_F"]]];
			waitUntil {
				_running = BRPVP_eventsDataCodeIsOn select _idx isEqualTo 2;
			...
			BRPVP_eventFortOfLightGroups pushBack _grpP;
			...
			BRPVP_eventFortOfLightObjects pushBack _heli;
			_heli setPos _start;
			_heli setDir _dir;
			...
			for "_i" from 1 to 3 do {
				...
				for "_j" from 1 to 5 do {
					...
					_unidade addEventHandler ["Killed",{BRPVP_eventFortOfLightDead pushBack (_this select 0);_this call BRPVP_botDaExp;}];
			...
			waitUntil {
				_aliveU = [];
				{_aliveU append units _x;} forEach BRPVP_eventFortOfLightGroups;
				_running = BRPVP_eventsDataCodeIsOn select _idx isEqualTo 2;
...
BRPVP_eventFortOfLightOff = {
	...
	if (_state isEqualTo 2) then {
		BRPVP_eventsDataCodeIsOn set [_idx,1];
		publicVariable "BRPVP_eventsDataCodeIsOn";
		{deleteWayPoint _x} forEach BRPVP_eventFortOfLightWayPoints;
		{
			_units = units _x;
			{
			...
			deleteGroup _x;
		} forEach BRPVP_eventFortOfLightGroups;
		{
			_x removeAllEventHandlers "HandleDamage";
			_x removeAllEventHandlers "Killed";
		...
		} forEach BRPVP_eventFortOfLightDead;
		{deleteVehicle _x;} forEach BRPVP_eventFortOfLightObjects;
		...
		{if (_x getVariable ["brpvp_yes_minerva",false]) then {deleteVehicle _x;};} forEach ((nearestObjects [_pos,["Building","Wall"],_rad,true])-BRPVP_eventFortOfLightObjects);
		...
		[_idx,+BRPVP_eventFortOfLightObjects] spawn BRPVP_eventRepositioneObjectsAtEnd;

		//CHG
		[] remoteExecCall ["BRPVP_AIRemoveNull",2];
		[] remoteExecCall ["BRPVP_updateAIUnitsArray",2];

		{[_x,false] remoteExecCall ["hideObjectGlobal",2];} forEach BRPVP_eventFortOfLightObjectsHide;
