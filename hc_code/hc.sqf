...
0 spawn {
	...
	BRPVP_pmiss2Objects = [];
	BRPVP_pmiss2ObjectsToDel = [];  //USADO PELA MISSAO SPECIAL FORCES
	...
	BRPVP_serverOrHcLoopCode = {
		...
		if (_contaD isEqualTo _loopsD) then {
			...
			//SEND HC FPS
			BRPVP_HCFps = round (_sFpsArray call BRPVP_arrayToFps);
			diag_log ("[HC1 FPS] "+str BRPVP_HCFps);
			publicVariable "BRPVP_HCFps";
			...
			private _newAiUnitsLocal = _newAiUnits select {local _x};
			_lastAllAiUnits = +_aiUnitsNow;
			{
				...
				_bot enableStamina false;
			} forEach _newAiUnitsLocal;

			//AMBOS
			//PLASMA SNIPER AI UNITS
			if (BRPVP_GX_M82A2000_SpawnTimes isNotEqualTo []) then {
				private _newAiUnitsNoVeh = _newAiUnitsLocal select {isNull objectParent _x && !(_x getVariable ["brpvp_is_ulfan",false])};
				private _plasmaDel = [];
				{
