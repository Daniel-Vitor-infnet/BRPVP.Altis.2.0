...
if (netId _lamp isEqualTo "0:0") then {
	if (typeOf _lamp in BRPVP_buildingHaveDoorListCVL) then {
		[typeOf _lamp,_lamp getVariable ["id_bd",-1],true] remoteExecCall ["BRPVP_saveLightStateDbsimpleObjectCVL",0];		
	} else {
	...
	if (isSimpleObject _lamp) then {
		...
		[_lamp,_lamp getVariable ["id_bd",-1],true] remoteExecCall ["BRPVP_saveLightStateDbsimpleObject",2];
	};
};
